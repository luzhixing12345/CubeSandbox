...
### A2. CubeNode Advanced DaemonSet Ready 数不足

先看三条 ADS（Big Pod / installer / bootstrap）与原生 PVM DaemonSet 的 desiredNumberScheduled 是否与实际节点数一致：

```bash
kubectl -n cube-system get daemonsets.apps.kruise.io \
  -l 'app.kubernetes.io/component in (cube-node,cube-node-installer,cube-node-bootstrap)'
kubectl -n cube-system get daemonset \
  -l 'app.kubernetes.io/component=cube-node-pvm'
```
...
```bash
kubectl -n cube-system logs cloneset/cube-api --tail=200
kubectl -n cube-system logs cloneset/cube-master -c cube-master --tail=200
```
...
1. 同一节点打两枚独立 label：`cube.tencent.com/cube-control=true` 与 `cube.tencent.com/cube-node=true`。不要再用同一个 `role` key 尝试同时写 `control`/`compute`，后写值会覆盖前值。
2. 使用 `values-single-node.yaml`，让控制面与计算面都容忍 `control`、`compute` 两把长期角色污点；仍可按标准语义保留两把污点。
3. 需要首次安装 PVM 时再加 `allow-pvm-bootstrap=true`。指纹未就绪时，Helm preflight Hook 写入临时门闩 `pvm-not-ready=true:NoSchedule`。

扩容出的纯计算节点只打 `cube-node=true` 与 `compute` 污点，不打 `cube-control=true`。控制面即使容忍 `compute` 污点，也会被 `cube-control` nodeSelector 锁在原混部节点。
...
```bash
kubectl -n cube-system exec cloneset/cube-master -- \
...
```bash
kubectl -n cube-system logs cloneset/cube-master -c cube-master | grep -i migrat
...
可以。Chart 交付的 `cubemastercli` CloneSet 已经内置一个 bashrc 函数,自动注入 `--address` / `--port`:

```bash
kubectl -n cube-system exec -it cloneset/cube-cubemastercli -- bash
# 然后直接输入
...
`pvm-host-bootstrap` 跑在 **`cube-node-pvm`** 原生 DaemonSet（仅 `placement.pvm` / `allow-pvm-bootstrap` 节点）。绝大多数反复重启是 host kernel 替换后节点需要 reboot。观察:
...
1. 检测 live kernel/boot args；若需 mutate，必须先确保 `pvm-not-ready` 污点，再通过 Eviction API 腾空本 namespace、本 release、本节点的依赖 Pod。ensure 或 eviction/PDB 失败会 fail-closed，禁止 reboot。
2. drain 成功后才 invalidate `node-prep-ready` / `pvm-host-ready` / `effective-pvm`，随后 acquire Lease、安装/配置 kernel 并触发 reboot。
3. 节点重启后 PVM DaemonSet 再次运行 → 快速路径写并复核 `pvm-host-ready` → 清除并复核临时污点。
4. bootstrap `wait-pvm-host` 校验指纹 → 写 `effective-pvm=1` → `cube-node-init` → 写 `node-prep-ready`。
5. Big Pod `wait-node-prep` sidecar（Kruise prio 10）只读 hostPath 自描述指纹，Ready 后主容器启动。
...
```bash
kubectl -n cube-system exec cloneset/cube-cubemastercli -- \
